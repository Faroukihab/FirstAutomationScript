...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestFacultiesPagewithDDT
    {
        ...
        public async Task TestUniCreation()
        {
            ...
            {
                string facultySearchName = data["facultySearchName"];
                string facultyEnglishName = data["facultyenglishName"];
                string facultyArabicName = data["facultyarabicName"];
                // Create page objects
                var faculty = new FacultiesPageWithDDT(page);

                // Perform methods
                await faculty.EnterFacultiescrud();
                await Task.Delay(1000);

                //await uni.SearchOnUni(UniSearchName);
                //await Task.Delay(1000);

                await faculty.CreateFaculty(facultyEnglishName, facultyArabicName);
                await Task.Delay(1000);
