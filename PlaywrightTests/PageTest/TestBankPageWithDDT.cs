...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestBanksPagewithDDT
    {
        ...
        public async Task TestBanksCreation()
        {
            ...
            {
                string BankSearchName = data["BankSearchName"];
                string BankEnglishName = data["BankEnglishName"];

                // Create page objects
                var bank = new BanksPageWithDDT(page);

                // Perform methods
                await bank.EnterBankPage();
                //Assert.AreEqual("http://185.227.111.191:666/general/banks", page.Url, "Page URL does not match the expected URL.");
                await Task.Delay(1000);

                await bank.SeachOnBanks(BankSearchName);
              

                await Task.Delay(2000);
                
                await bank.CreateNewBank(BankEnglishName);
                await Task.Delay(2000);
