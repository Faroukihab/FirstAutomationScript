...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestInsuranceCompanywithDDT
    {
        ...
        public async Task TestUniCreation()
        {
            ...
            {
                string InsuranceCompanyArabicName = data["InsuranceCompanyArabicName"];
                string InsuranceCompanyEnglishName = data["InsuranceCompanyEnglishName"];
                string InsuranceCompanySearchName = data["InsuranceCompanySearchName"];
                // Create page objects
                var Ins = new InsuranceCompanieswithDDT(page);

                // Perform methods
                await Ins.Enterinsurancecompanies();
                await Task.Delay(1000);

                //await uni.SearchOnUni(UniSearchName);
                //await Task.Delay(1000);

                await Ins.Createinsurancecompanies(InsuranceCompanyArabicName, InsuranceCompanyEnglishName);
                await Task.Delay(1000);
