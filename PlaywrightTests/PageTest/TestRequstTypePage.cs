...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestRequestTypepage
    {
        ...
        public async Task TestRequstTypeCreation()
        {
            ...
            {
                string RequestSearchName = data["RequestSearchName"];
                string BankTransactionTypesArabicName = data["RequstArabicName"];
                string BankTransactionTypesEnglishName = data["RequstEnglishName"];
                string responseTime = data["ResponseTime"];

                // Create page objects
                var requesttyp = new RequestTypeWithDDT(page);

                // Perform methods
                await requesttyp.EnterRequestTypeCrude();
                ...
                await requesttyp.SearchonCreatedRequested(RequestSearchName);
                await Task.Delay(2000);

                await requesttyp.CreateaRequest(BankTransactionTypesEnglishName, BankTransactionTypesArabicName, responseTime);
                await Task.Delay(2000);
