...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestCleintPagewithDDT
    {
        ...
        [Test]
        public async Task TestCleintCreation()
        {
            var testData = GetTestData();

            foreach (var data in testData)
            {
                string CleintSearchName = data["CleintSearchName"];
                string CleintArabicName = data["CleintArabicName"];
                string ClientEnglishName = data["ClientEnglishName"];
                string CleintPrefix = data["CleintPrefix"];
                string CleintCommRegis = data["CleintCommRegis"];
                string CleintVat = data["CleintVat"];
                string CleintNumber = data["CleintNumber"];
                

                // Create page objects
                var cleint = new CleintPageWithDDT(page);

                // Perform methods
                await cleint.EnterClientCrud();
                //Assert.AreEqual("http://185.227.111.191:666/general/banks", page.Url, "Page URL does not match the expected URL.");
                await Task.Delay(1000);

                await cleint.Search(CleintSearchName);
                await Task.Delay(2000);

                await cleint.CreateClient(CleintArabicName, ClientEnglishName, CleintPrefix, CleintCommRegis, CleintVat, CleintNumber);
                await Task.Delay(2000);
