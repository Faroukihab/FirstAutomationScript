...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestBankTransactionTypepage
    {
        ...
        [Test]
        public async Task TestBankTransactionTypeCreation()
        {  
            var testData = GetTestData();

            foreach (var data in testData)
            {
                string banktransactiontypesearch = data["banktransactiontypesearch"];
                string BankTransactionTypesArabicName = data["BankTransactionTypesArabicName"];
                string BankTransactionTypesEnglishName = data["BankTransactionTypesEnglishName"];

                // Create page objects
                var banktrans = new BankTransactionTypesWithDDT(page);

                // Perform methods
                await banktrans.Enterbanktransactiontypes();
                ...
                await banktrans.SeachOnbanktransactiontypes(banktransactiontypesearch);
                await Task.Delay(2000);

                await banktrans.CreateNewbanktransactiontypes(BankTransactionTypesArabicName, BankTransactionTypesEnglishName);
                await Task.Delay(2000);
