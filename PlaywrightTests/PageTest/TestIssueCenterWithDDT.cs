...
namespace TF.PageTest
{
    ...
    [TestFixture]
    public class TestIssueCenterwithDDT
    {
        ...
        [Test]
        public async Task TestLaborOfficeCreation()
        {
            var testData = GetTestData();

            foreach (var data in testData)
            {
                string IssueCenterEnglishName = data["IssueCenterEnglishName"];
                string IssueCenterArabicName = data["IssueCenterArabicName"];

                // Create page objects
                var issue = new IssueCenterWithDDT(page);

                // Perform methods
                await issue.EnterIssueCenterPage();
                //Assert.AreEqual("http://185.227.111.191:666/general/banks", page.Url, "Page URL does not match the expected URL.");
                await Task.Delay(1000);

                await issue.CreateIssueCenter(IssueCenterEnglishName,IssueCenterArabicName);
                await Task.Delay(2000);

                //await issue.SeachOnIssueCenter("IsssueCenterSearchName");
                //await Task.Delay(2000);
