...
public class CivilOfficces
{
    ...
    public CivilOfficces(IPage page)
    {
        _page = page;
    ...
    public async Task CreateCivilOfice()
    {
        await CreatBtn.ClickAsync();
        await _page.GetByLabel("Arabic Name *").ClickAsync();

        await _page.GetByLabel("Arabic Name *").FillAsync("ت2");

        await _page.GetByLabel("Arabic Name *").PressAsync("Tab");

        await _page.GetByLabel("English Name *").FillAsync("t2");
